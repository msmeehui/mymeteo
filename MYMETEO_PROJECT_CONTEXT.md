...
The radar slider has a compact precipitation timeline above the range input. It should show the expected local precipitation shape for the same slider range with a soft filled graph and dry/light/moderate/heavy scale, but avoid visible millimeters-per-hour values because they add precision without much everyday feel. Keep the graph separate from the slider thumb so a finger does not block the precipitation shape while scrubbing. The graph and selected-time marker should use the same thumb-center track and the same canonical selected-time precipitation as the weather card. Show the selected time with one dashed vertical line meeting the top of the large slider thumb; omit the separate selected-value dot to keep the control visually simple. Inside visible radar coverage, the graph should be radar-map-first: sample the same KNMI/Buienradar/LibreWXR radar image frames at the selected location and let those frame samples decide the dry/light/moderate/heavy shape. Use point/model data only as fallback or beyond reliable radar-image coverage, so the timeline does not visibly contradict the radar map. When the local radar image is explicitly dry inside the radar coverage window, the graph should stay visually dry rather than showing a leftover point/model drizzle hint. The Heavy guide line should be a heavy-rain threshold, not the ceiling of the graph; stronger heavy precipitation may rise into the small breathing space above the line.
...
The About/Data Sources section and README explain that the selected Today time follows the displayed KNMI/Buienradar image when it can be read at the location, with KNMI point rain and then Buienradar/Open-Meteo as fallback and longer-range guidance.
...
After live Amsterdam testing in heavy rain and thunderstorms, KNMI became the primary Netherlands near-term rain source. KNMI drives the first two hours of the visible radar through the authenticated proxy. At the selected Today time, the displayed KNMI image drives the weather card, rain chance, and rain icon; the KNMI point reading is the preferred fallback when that image cannot be sampled. Buienradar remains the longer-range/fallback source: after KNMI's two-hour radar window, the slider continues with Buienradar frames, and the 3-hour/8-hour Buienradar control still determines the extended range. Earlier investigation found the public no-key Buienradar animation endpoint capped at about 36 five-minute frames for the simple 3-hour setup; longer horizons generally require model forecasts, different providers, or later source changes.

KNMI map images must be requested in EPSG:3857 because Leaflet displays the rectangular overlay in Web Mercator. Exact-location image sampling must use that same projected geometry. KNMI point queries remain EPSG:4326. Keep valid time, reference run, browser-fetch time, proxy cache age/state, CRS, exact-location signal, and nearby-context signal available in hidden rain diagnostics so apparent spatial or timing disagreements can be compared at the same location and instant.

For locations outside the Netherlands or when KNMI/Buienradar are unavailable, LibreWXR/fallback radar tiles are used. The outside-Netherlands one-hour radar animation was smoothed to feel less jumpy. LibreWXR now follows the same local-rain rule as the Dutch imagery: sample the four source pixels surrounding the marker, interpolate only the actual lower/upper frames, and feed that state to the Today card, rain icon, graph and outfit. Sampled tiles are reused as object URLs on the map, with native zoom fixed at 7 through the existing zoom range, so zoom and duplicate downloads cannot change their meaning. Retain the prior display until replacement map layers and local samples have settled; failed map frames and obsolete requests must not switch the selected state.

LibreWXR's transparent pixels can mean dry or missing data. Its coverage PNG only marks precipitation, so do not use it as evidence of dry coverage. Accept transparent readings only when a contemporaneous successful health response confirms the loaded global IFS fallback and agrees with the accepted current/nowcast frame catalog. The provider has no atomic generation token across health and tile requests; this is conservative availability evidence, not a ground-truth guarantee. Missing tiles, unreadable images or unknown coverage remain unknown. If any frame lacks a local reading, withhold the fine-grained LibreWXR graph and show the compact `Local rain detail unavailable` status; the card can use forecast fallback. Do not connect a curve across missing frames or turn a positive hourly rain probability into an apparently continuous radar rain band.
